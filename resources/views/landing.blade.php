...
<x-guest-layout>
            ...
            <div>
                ...
                <div class="hero-cta mt-8 flex flex-wrap gap-4">
                    <a href="{{ url('/newcomplaint') }}"
                        class="text-emerald inline-flex items-center gap-2 px-7 py-4 rounded-full font-semibold text-sm tracking-wide">
                ...
                <div class="flex items-center gap-10 px-6">
                    @foreach (['Confidential', 'Transparent', 'Accountable', 'Responsive', 'Citizen-First', '72-Hour Response', 'Fair Resolution'] as $w)
                        <span class="flex items-center gap-3"><i data-lucide="leaf"
                                class="w-3.5 h-3.5 text-da-gold"></i>
                            {{ $w }}</span>
                    @endforeach
                </div>
            @endfor
                ...
                <div class="relative">
                    ...
                    <div class="mt-6 space-y-4 text-sm">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <a href="mailto:ored@mimaropa.da.gov.ph" class="hover:underline">ored@mimaropa.da.gov.ph</a>
                        </div>
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-lg bg-white/15 flex items-center justify-center"><i
    ...
    <section class="py-20 bg-gradient-to-b from-white to-da-soft-green/30" id="suggestions">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

            {{-- Section header --}}
            <div class="text-center max-w-2xl mx-auto mb-12 reveal">
                <p class="text-xs font-bold tracking-[0.25em] text-da-green uppercase">Your Voice Matters</p>
                <h2 class="mt-3 text-3xl sm:text-4xl font-extrabold text-gray-900">
                    Suggestions &amp; Recommendations
                </h2>
                <p class="mt-4 text-gray-600 text-sm leading-relaxed">
                    Help us serve you better. Share your ideas, suggestions, or recommendations to improve
                    DA MIMAROPA's programs, services, and processes. Anonymous submissions are welcome.
                </p>
            </div>

            <div class="grid lg:grid-cols-5 gap-10 items-start">

                {{-- ---- Left: Why it matters cards ---- --}}
                <div class="lg:col-span-2 space-y-4 reveal">
                    @php
                        $pillars = [
                            [
                                ...
                                'desc' =>
                                    'Your insights help us refine DA programs and make public service more efficient for farmers and stakeholders.',
                                'bg' => 'da-soft-green',
                                'color' => 'da-green',
                            ],
                            [
                                ...
                                'desc' =>
                                    'Sharing your name and designation is completely optional. Your recommendation is what matters most.',
                                'bg' => 'da-soft-blue',
                                'color' => 'blue-700',
                            ],
                            [
                                'icon' => 'check-circle-2',
                                'title' => 'Reviewed & Acted Upon',
                                'desc' =>
                                    'Every suggestion is read by our team. Feasible ideas are escalated to the appropriate DA office.',
                                'bg' => 'da-soft-amber',
                                'color' => 'amber-700',
                            ],
                        ];
                    ...
                    @foreach ($pillars as $i => $p)
                        <div
                            class="reveal reveal-delay-{{ $i + 1 }} lift flex items-start gap-4 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                            <div
                                class="w-11 h-11 rounded-xl bg-{{ $p['bg'] }} flex items-center justify-center text-{{ $p['color'] }} shrink-0">
                                <i data-lucide="{{ $p['icon'] }}" class="w-5 h-5"></i>
                            </div>
                            <div>
                                <p class="font-semibold text-gray-900 text-sm">{{ $p['title'] }}</p>
                                <p class="text-xs text-gray-500 mt-1 leading-relaxed">{{ $p['desc'] }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="lg:col-span-3 reveal reveal-delay-2">
                    <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 lg:p-10">
                        ...
                        @if (session('success'))
                            <div
                                class="mb-6 flex items-start gap-3 bg-da-soft-green border border-da-green/20 rounded-xl px-4 py-3 text-da-green text-sm">
                        ...
                        @if ($errors->any())
                            <div
                                class="mb-6 flex items-start gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-red-700 text-sm">
                        ...
                        <form action="{{ route('suggestions.store') }}" method="POST" id="suggestionForm">
                            ...
                            {{-- Optional: Name & Designation (side by side) --}}
                            ...
                            <div class="grid sm:grid-cols-2 gap-4 mb-6">
                                {{-- Name --}}
                                <div>
                                    <label for="sugg_name" class="block text-xs font-semibold text-gray-600 mb-1.5">
                                        Full Name
                                    </label>
                                    <div class="relative">
                                        <span
                                        ...
                                        <input type="text" id="sugg_name" name="name"
                                            value="{{ old('name') }}" placeholder="e.g. Juan Dela Cruz"
                                            class="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-da-green/30 focus:border-da-green transition bg-gray-50 placeholder-gray-400">
                                    </div>
                                </div>

                                {{-- Designation --}}
                                <div>
                                    <label for="sugg_designation"
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text" id="sugg_designation" name="designation"
                                            value="{{ old('designation') }}"
                                            placeholder="e.g. Farmer, LGU Staff, DA Employee"
                                            class="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-da-green/30 focus:border-da-green transition bg-gray-50 placeholder-gray-400">
                                    </div>
                                </div>
                            </div>
                            ...
                            {{-- Category --}}
                            <div class="mb-5">
                                <label for="sugg_category" class="block text-xs font-semibold text-gray-600 mb-1.5">
                                    Category
                                ...
                                <div class="relative">
                                    ...
                                    <select id="sugg_category" name="category"
                                        class="w-full pl-9 pr-8 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-da-green/30 focus:border-da-green transition bg-gray-50 appearance-none">
                                        <option value="other"
                                            {{ old('category', 'other') === 'other' ? 'selected' : '' }}>Select a
                                            category…</option>
                                        <option value="service_improvement"
                                            {{ old('category') === 'service_improvement' ? 'selected' : '' }}>Service
                                            Improvement</option>
                                        <option value="policy" {{ old('category') === 'policy' ? 'selected' : '' }}>
                                            Policy
                                        </option>
                                        <option value="staff_behavior"
                                            {{ old('category') === 'staff_behavior' ? 'selected' : '' }}>Staff
                                            Behavior</option>
                                        <option value="facilities"
                                            {{ old('category') === 'facilities' ? 'selected' : '' }}>Facilities
                                        </option>
                                        <option value="processes"
                                            {{ old('category') === 'processes' ? 'selected' : '' }}>
                                            Processes
                                        </option>
                                    </select>
                                    <span
                            ...
                            {{-- Suggestion / Recommendation — REQUIRED --}}
                            <div class="mb-6">
                                <label for="sugg_body" class="block text-xs font-semibold text-gray-600 mb-1.5">
                                    Your Suggestion or Recommendation
                                    <span class="text-red-500 ml-0.5">*</span>
                                </label>
                                <textarea id="sugg_body" name="suggestion" rows="5"
                                    placeholder="Share your ideas to improve our services, programs, processes, or any other aspect of DA MIMAROPA…"
                                    maxlength="3000" required
                                    class="w-full px-4 py-3 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-da-green/30 focus:border-da-green transition bg-gray-50 placeholder-gray-400 resize-none @error('suggestion') border-red-400 @enderror">{{ old('suggestion') }}</textarea>
                                <div class="flex justify-between items-center mt-1.5">
                                    <p class="text-xs text-gray-400">Minimum 10 characters</p>
                                    <p class="text-xs text-gray-400">
                                        <span id="suggCharCount">0</span> / 3,000
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                <p class="text-xs text-gray-400 flex items-center gap-1.5">
                                    <i data-lucide="lock" class="w-3.5 h-3.5 text-da-green shrink-0"></i>
                                    Name &amp; designation are never published without your consent.
                                </p>
                                <button type="submit"
                                    class="btn-primary inline-flex items-center gap-2 px-7 py-3 rounded-full text-sm font-semibold tracking-wide whitespace-nowrap"
                                    style="--da-green:#1f6b3a; background:linear-gradient(135deg,#1f6b3a,#2f9d5a);">
